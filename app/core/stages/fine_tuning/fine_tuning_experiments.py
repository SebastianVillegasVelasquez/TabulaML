...
def get_fine_tuning_experiments(context: RunContext):
    """
    ...
    param: context: RunContext object containing the stage results.
    """
    ...
    for model_name, model_results in stage_results.metadata['top_k_models_by_family'].items():
        ...
        tuner = FineTunerFactory.create_tuner(tuner_strategy=tuner_strategy,
                                              context=context)
        try:
