...
class NumericalFeature(FeatureConfig):
    ...
    Encapsulates the descriptive statistics needed to automatically select
    the most appropriate transformation and scaling strategy for the
    preprocessing pipeline.
 
    The transformation selection logic follows this priority hierarchy:
        1. Negative values present -> YEO_JOHNSON (only safe option).
        2. Low skewness (near-normal distribution) -> STANDARD or ROBUST.
        3. Zeros present -> LOG1P or YEO_JOHNSON (standard LOG does not apply).
        4. High skewness, no zeros, no negatives -> LOG or BOX_COX.
        5. Severe outliers -> ROBUST.
 
    Attributes:
        skewness: Distribution asymmetry. Positive = right tail, negative = left tail.
        zero_ratio: Proportion of values exactly equal to zero [0, 1].
        has_negative_values: True if the column contains any negative value.
        outlier_ratio: Proportion of detected outliers (e.g., via IQR or z-score) [0, 1].
        variance: Column variance. Useful for detecting near-zero variance features.
        is_discrete: True if the column takes countable integer values (e.g., counts).
        suggested_imputation: Recommended imputation strategy based on the distribution.
 
    Raises:
        ValueError: If zero_ratio or outlier_ratio are not in [0, 1].
    """
 
    feature_type: FeatureType = Field(default=FeatureType.NUMERICAL)
    subtype: NumericalSubtype = Field(default=NumericalSubtype.CONTINUOUS)
    skewness: float = Field(..., description="Distribution skewness coefficient.")
    zero_ratio: float = Field(default=0.0, ge=0.0, le=1.0, description="Proportion of zeros.")
    has_negative_values: bool = Field(
        default=False, description="True if the column contains negative values."
    )
    outlier_ratio: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Proportion of detected outliers."
    )
    variance: Optional[float] = Field(
        default=None, description="Column variance. None if not computed."
    )
    is_discrete: bool = Field(
        default=False, description="True if values are countable integers."
    )
    suggested_imputation: ImputationStrategy = Field(
        default=ImputationStrategy.MEDIAN,
        description="Recommended imputation strategy.",
    )
 
    @property
    def is_skewed(self) -> bool:
        """Detects whether the distribution has significant asymmetry.
 
        Uses the conventional threshold of |skewness| > 1.0 to classify
        a distribution as highly skewed.
 
        Returns:
            True if the absolute skewness value exceeds 1.0.
        """
        return abs(self.skewness) > 1.0
 
    @property
    def is_near_zero_variance(self) -> bool:
        """Detects whether the feature has practically zero variance.
 
        Near-zero variance features contribute little predictive information
        and should generally be dropped from the pipeline.
 
        Returns:
            True if variance is below the 1e-6 threshold, or if not computed.
        """
    ...
    def suggested_transformation(self) -> TransformationType:
        """Selects the optimal transformation based on subtype and distribution stats.

        Subtype-based rules take full priority over distribution heuristics,
        since the structural nature of the column (cyclic, count, binary) imposes
        hard mathematical constraints that skewness alone cannot capture.

        Priority order:
            1. BINARY_ENCODED → NONE (already a 0/1 flag)
            2. CYCLIC → NONE (pipeline handles sin/cos separately)
            3. ORDINAL_ENCODED → NONE (scaling would destroy ordinal meaning)
            4. COUNT → LOG1P if zeros present, LOG otherwise
            5. CONTINUOUS → distribution-based logic (skewness + outliers)

        Returns:
            The most appropriate TransformationType for this feature.
        """
        # --- Subtype-driven rules (structural constraints, highest priority) ---

        if self.subtype == NumericalSubtype.BINARY_ENCODED:
            return TransformationType.NONE

        if self.subtype == NumericalSubtype.CYCLIC:
            return TransformationType.NONE  # sin/cos handled by a dedicated transformer

        if self.subtype == NumericalSubtype.ORDINAL_ENCODED:
            return TransformationType.NONE  # preserve ordinal meaning as-is

        if self.subtype == NumericalSubtype.COUNT:
            # log1p is the natural transform for count data: handles zeros safely
            # and compresses the heavy Poisson right tail
            return TransformationType.LOG1P if self.zero_ratio > 0 else TransformationType.LOG

        # --- Distribution-based rules for CONTINUOUS columns ---

        if self.has_negative_values:
            return TransformationType.YEO_JOHNSON

        if not self.is_skewed:
            return TransformationType.ROBUST if self.outlier_ratio > 0.05 else TransformationType.STANDARD
