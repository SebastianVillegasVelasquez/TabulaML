...
class TestModelSelectionEvaluatorEdgeCases:
    ...
    def minimal_setup(self):
        ...
        evaluator = ModelSelectionEvaluator(stage=Stages.MODEL_SELECTION, context=context)
    ...
    def test_models_by_family_count(self, minimal_setup):
        ...
        for model, metric in configs:
            model_class = (
                LogisticRegression() if model == "LogisticRegression" else RandomForestClassifier()
            )
            result = ExperimentResult(
                name=f"exp_{metric}",
...
class TestModelSelectionEvaluatorModelFamilyExtraction:
    ...
    def test_different_families_recognized(self, setup):
        ...
        result2 = ExperimentResult(name="test2", pipeline=Pipeline([]), config={"model": "SVM"})
