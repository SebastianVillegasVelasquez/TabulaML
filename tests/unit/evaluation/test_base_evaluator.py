...
class TestBaseEvaluator:
    ...
    def test_evaluator_initialization(self, mock_context):
        ...
        evaluator = ConcreteEvaluator(stage=Stages.FEATURE_SELECTION, context=mock_context)
    ...
    def test_evaluate_calls_template_methods(self, evaluator, sample_results):
        ...
        evaluator._extract_stage_specific_data = MagicMock(return_value={"test": "data"})
    ...
    def test_extract_top_k_by_family_single_per_family(self, evaluator):
        ...
        sorted_results = sorted(results, key=lambda r: r.metrics["test_accuracy"], reverse=True)
    ...
    def test_sort_results_consistency(self, evaluator):
        ...
        for i in range(5):
            result = ExperimentResult(
                name=f"exp_{i}", pipeline=Pipeline([]), metrics={"test_accuracy": 0.8 + (i * 0.03)}
            )
            results.append(result)
...
class TestEvaluatorWithMultipleScoringMetrics:
    ...
    def test_sort_uses_first_metric_from_list(self, multi_metric_context):
        ...
        evaluator = ConcreteEvaluator(stage=Stages.MODEL_SELECTION, context=multi_metric_context)
